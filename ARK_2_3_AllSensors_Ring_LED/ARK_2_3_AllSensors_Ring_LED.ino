...
const int kMaxLED    = 128;  //  最大の明るさ
const int kWS2811Pin = 14;  //  DINへつなぐIO
Adafruit_NeoPixel WS = Adafruit_NeoPixel(60, kWS2811Pin, NEO_RGB + NEO_KHZ800);
...
void loop() {
  ...
  //  WS2811を表示更新
  //
  wsRed   = hokan(temperature, 28, 32);
  wsGreen = (60 - pirCounter) / 6.0;
  wsBlue  = hokan(humidity, 40, 30);
  
  WS.setPixelColor(0, WS.Color(wsRed, wsGreen, wsBlue));
  WS.show();
...
int hokan(float inValue, int inLow, int inHigh) {
  int result;
  if (inLow < inHigh) {
    result = (inValue - (float)inLow) / (float)(inHigh-inLow) * (float)kMaxLED;
  } else {
    result = ((float)inLow - inValue) / (float)(inLow-inHigh) * (float)kMaxLED;
  }
  if (result < 0) result = 0;
  if (result > kMaxLED) result = kMaxLED;
...
void timerCountPIR(void *temp) {
  ...
  int pirStatus = digitalRead(kPIRSensor);
  if (pirStatus == HIGH) {
    data[s] = 1;
  } else {
    data[s] = 0;
